...
import pytest
from app.models import Room, Band, Booking, convert_quickstudio_response
from app.quickstudioapi import RoomBooking, Booking as QuickBooking, Band as QuickBand
import json

def test_room_hash():
    room1 = Room(id=1, name="Room 1", description="Desc 1", size=10, open=time(9, 0), close=time(17, 0), studio_name="Studio A")
    room2 = Room(id=1, name="Room 1", description="Desc 1", size=10, open=time(9, 0), close=time(17, 0), studio_name="Studio A")
...
def test_convert_quickstudio_response_no_band_exception():
    ...
    with pytest.raises(Exception, match="Booking for room 'Room 1' has no band but has type 1"):
...
def test_convert_quickstudio_response_booking_spans_several_days_exception():
    ...
    with pytest.raises(Exception, match="booking .* spans several days which is not expected"):
        convert_quickstudio_response("test_studio", room_bookings)
